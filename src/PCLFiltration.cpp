...
void PCLFiltration::onInit() {
  ...
  if (_depth_republish) {
    ...
    _pub_depth_up                = nh.advertise<sensor_msgs::PointCloud2>("depth_up_out", 10);
    _pub_depth_over_max_range_up = nh.advertise<sensor_msgs::PointCloud2>("depth_up_over_max_range_out", 10);

    _sub_depth_down                = nh.subscribe("depth_down_in", 1, &PCLFiltration::depthCallback, this, ros::TransportHints().tcpNoDelay());
    _pub_depth_down                = nh.advertise<sensor_msgs::PointCloud2>("depth_down_out", 10);
    _pub_depth_over_max_range_down = nh.advertise<sensor_msgs::PointCloud2>("depth_down_over_max_range_out", 10);
  }
